...
class BoltTests(unittest.TestCase):
    def setUp(self):
        ...
        self.bolt = Bolt(input_stream=BytesIO(tup_json), output_stream=BytesIO())
...
class BatchingBoltTests(unittest.TestCase):
    ...
    def test_auto_ack_off(self, ack_mock):
        ...
        # otherwise.
        ack_mock.assert_has_calls(
            [mock.call(self.bolt, tup) for tup in self.tups if self.bolt.is_tick(tup)],
            any_order=True,
        )
    ...
    def test_auto_fail_partial_exit_on_exception_true(
        self, fail_mock, process_batch_mock
    ):
        # Need to re-register signal handler with mocked version, because
        # mock gets created after handler was originally registered.
        self.setUp()
